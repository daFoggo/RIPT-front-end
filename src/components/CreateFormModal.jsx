...
const CreateFormModal = (props) => {
  function createForm() {
    props.handleCloseCreateFormModal();
      ...
      <div className="flex flex-col w-1/3 bg-white p-5 rounded-md">
        <div className="header flex justify-between pb-3 border-b-2">
          ...
          <svg
            ...
            stroke="currentColor"
            className="w-6 h-6"
            onClick={props.handleCloseCreateFormModal}
          >
            <path
              strokeLinecap="round"
        ...
        <div className='content flex flex-col mt-3 gap-3 border-b-2 pb-3'>
          <div className='flex flex-col gap-1'>
            <select name="" id="formTypeSelect" className='bg-[#e4efff] p-2 rounded-md w-1/3 border-[#bfd6fd] border-2'>
              <option value="0" selected>Chọn loại đơn</option>
              <option value="1">Đơn xin nghỉ</option>
              <option value="2">Đơn xin đến muộn</option>
              <option value="3">Đơn xin về sớm</option>
            </select>
          </div>
...
CreateFormModal.propTypes = {
  handleCloseCreateFormModal: PropsType.func
}
